...
const IncidentWizard = () => {
        ...
        >
          <div className="space-y-2">
            ...
            <GuidedDetailsForm
              ...
              value={details}
              onChange={(value) => {
                // Ensure the details are correctly updated when guided form changes
                console.log("Details updated:", value);
                setDetails(value);
              }}
              onToggleGuided={(isGuided) => {
                console.log("Guided mode toggled:", isGuided);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        // Log the event through analytics service
      ...
      setError(`Failed to submit the incident report: ${error.message || "Please try again."}`);
    } finally {
